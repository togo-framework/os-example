...
import (
	...
	"github.com/togo-framework/os-example/internal/app"
	_ "github.com/togo-framework/os-example/internal/admin" // mounts the built-in /api/admin user-management + mail surface
	_ "github.com/togo-framework/autopilot" // ships the Autopilot board + Issues->Agent->Code loop at /autopilot
	graphgen "github.com/togo-framework/os-example/internal/graph/gen"
	"github.com/togo-framework/os-example/internal/graph/resolvers"
	_ "github.com/togo-framework/os-example/internal/plugins" // blank-imports installed plugins (togo install)
	"github.com/togo-framework/os-example/internal/rest"
)
...
func Boot() *app.App {
	...
	api := humachi.New(k.Router, huma.DefaultConfig("OsExample API", "0.1.0"))
	...
	k.Router.Handle("/graphql/play", playground.Handler("OsExample GraphQL", k.Config.GraphQLPath))

	k.Router.Get("/api/health", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprintf(w, `{"status":"ok","service":"os-example","togo":%q}`, togoVersion())
	})
...
func OpenAPI() ([]byte, error) {
	router := chi.NewMux()
	api := humachi.New(router, huma.DefaultConfig("OsExample API", "0.1.0"))
